      ...
      <el-table-column label="货主名称" prop="id" sortable="custom" align="left">
        <template slot-scope="scope">
          ...
          <div class="topBorder">
            ...
            <span :xs="8" :sm="6" :md="4" :lg="4" :xl="1"><i>更新时间：</i>{{ scope.row.updateDate | Timestamp }}</span>
            <span v-if="Number(scope.row.isPayDeposit)" :xs="8" :sm="6" :md="4" :lg="4" :xl="1"><i>是否缴纳保证金：</i>是</span>
            <span v-if="!Number(scope.row.isPayDeposit)" :xs="8" :sm="6" :md="4" :lg="4" :xl="1"><i>是否缴纳保证金：</i>否</span>
            <span :xs="8" :sm="6" :md="4" :lg="4" :xl="1"><i>拓展经理：</i>{{ scope.row.expandManager }} / {{ scope.row.bindTypeName }}<i v-if="scope.row.creatorName">（{{ scope.row.creatorName }}）</i></span>
          </div>
          <el-row :gutter="30">
            <el-col :span="6">
            ...
            <el-col :span="7">
              ...
              <p>最后登陆时间： {{ scope.row.lastLoginTime | Timestamp }}</p>
            </el-col>
            <el-col :span="5">
              ...
              <p>来源小程序： {{ scope.row.appletSourceName | DataIsNull }}</p>
            </el-col>
            <el-col :span="4" class="btnBox">
              ...
              <el-button v-permission="['/driver/driver/clue/bss/clueInfo']" type="text" @click="goDetals(scope.row)">详情</el-button>
          ...
          <el-row :gutter="30">
            ...
            <el-col :span="3" class="btnBox">
              <el-button v-permission="['/driver/driver/clue/bss/create']" v-if="(scope.row.state === 1 || scope.row.state === 2) && scope.row.workCityName" class="inline" type="text" @click="addClue(scope.row.clueId)">跟进</el-button><el-button v-permission="['/driver/driver/clue/bss/clueInfo']" class="inline" type="text" @click="goDetals(scope.row)">详情</el-button>
              <el-button v-permission="['/driver/driver/clue/bss/clueDistribute']" type="text" @click="distributionClue(scope.row.clueId)">线索分配</el-button>
              <!-- <el-button v-permission="['/driver/driver/clue/log/list']" type="text" @click="goLog(scope.row.clueId)">操作日志</el-button> -->
            </el-col>
          </el-row>
...
import { getClueList, exportExcelClueList, getBDManager } from '@/api/clue/clue-api'
...
export default {
  ...
  mounted() {
    if (this.$route.query && this.$route.query.id) {
      this.listQuery.key = this.$route.query.id
    }
  ...
  methods: {
    ...
    goDetals(row) {
      ...
      if (row.userId) {
        query.driverId = row.userId;
      }
      if (this.$store.getters.newTab) {
        let routeData = this.$router.resolve({ path: '/clue-detail', query })
        window.open(routeData.href, '_blank');
      } else {
        this.$router.push({ path: '/clue-detail', query })
      }
    },
    goLog(id) {
      if (this.$store.getters.newTab) {
