    ...
    <sticky class-name="sub-navbar">
      <div class="filter-container">
        <div class="menuBox">
          ...
          <el-button
            v-permission="['/line/line/task/createLineTaskClue']"
            class="filter-item"
            style="float: right;"
            type="primary"
            icon="el-icon-add"
            @click="handleCreate">创建线路线索
          </el-button>
          <!--<el-button-->
          <!--class="filter-item"-->
          <!--style="float: right;"-->
        ...
        <div>
          <el-badge :value="listQuery.state === 1 ? totalDetail.all : ''" class="item" type="primary">
            <el-button :plain="listQuery.state !== 1" type="primary" size="small" @click="stateChange(1)">当前待审核线路数</el-button>
          </el-badge>
          <el-badge :value="listQuery.state === 3 ? totalDetail.all : ''" class="item" type="primary">
            <el-button :plain="listQuery.state !== 3" type="primary" size="small" @click="stateChange(3)">集团线路待确认数</el-button>
          </el-badge>
          <el-badge :value="listQuery.state === 4 ? totalDetail.all : ''" class="item" type="primary">
            <el-button :plain="listQuery.state !== 4" type="primary" size="small" @click="stateChange(4)">超时未操作</el-button>
          </el-badge>
        </div>
      </div>
    </sticky>
      ...
      <el-table-column label="货主名称" prop="id" sortable="custom" align="left">
        <template slot-scope="scope">
          ...
          <div class="topBorder">
            ...
            <span :xs="8" :sm="6" :md="4" :lg="4" :xl="1"><i>创建时间：</i>{{ scope.row.createDate | Timestamp }}<i v-if="scope.row.creatorName">（{{ scope.row.creatorName }}）</i> <i v-if="Number(scope.row.primaryClassification) === 3 && scope.row.isOverdueOpt" style="float:right;">超时未操作</i></span>
          ...
          <el-row :gutter="30">
            ...
            <el-col :span="3" class="btnBox">
              <el-button v-permission="['/bss/v1/bss/line/task/selectListByQuery/applyNotPass', '/line/line/task/inspectPass', '/line/line/task/inspectNotPass']" v-if="scope.row.selfState !== 0 && scope.row.state === 1" type="primary" plain size="mini" @click="goAudit(scope.row.lineId, scope.row.tenderId, scope.row.timeDiff, scope.row.monthlyTransaction)">审核</el-button>
              <el-button v-permission="['/line/line/task/edit']" v-if="scope.row.selfState !== 0 && (scope.row.state === 1 || scope.row.state === 2)" type="primary" plain size="mini" style="margin-top: 10px !important;" @click="editDetails(scope.row.lineId)">{{ $t('table.edit') }}</el-button>
              <el-button v-permission="['/line/line/task/confirmLineTaskClue']" v-else-if="scope.row.state === 4" type="primary" plain size="mini" style="margin-top: 10px !important;" @click="goAudit2(scope.row.lineId, scope.row.customerId)">线路确认</el-button>
            </el-col>
          </el-row>
        </template>
      </el-table-column>
...
import { fetchListWaitApply, getTenderTime } from '@/api/carline/carline-api'
...
export default {
  ...
  methods: {
    ...
    goAudit(id, id2, timeDiff, monthlyTransaction) {
      if (this.$store.getters.newTab) {
        let routeData = this.$router.resolve({ path: '/tender-audit', query: { id: id, tenderId: id2, timeDiff: timeDiff, monthlyTransaction: monthlyTransaction }})
        window.open(routeData.href, '_blank');
      } else {
        this.$router.push({ path: '/tender-audit', query: { id: id, tenderId: id2, timeDiff: timeDiff, monthlyTransaction: monthlyTransaction }})
      }
    },
    goAudit2(id, id2) {
      if (this.$store.getters.newTab) {
        let routeData = this.$router.resolve({ path: '/add-line-group', query: { id: id, customerId: id2 }})
        window.open(routeData.href, '_blank');
      } else {
        this.$router.push({ path: '/add-line-group', query: { id: id, customerId: id2 }})
      }
    },
    getList($state) {
      this.listLoading = true
      fetchListWaitApply(this.listQuery).then(response => {
    ...
    handleCreate() {
      this.$router.push({ path: '/add-line-clue' })
    },
    detetTemp() {
      this.temp = {
        driver: '',
