      ...
      <div class="filter-container">
        ...
        <div>
          <el-badge :value="listQuery.state === 1 ? totalDetail.all : ''" class="item" type="primary">
            <el-button :plain="listQuery.state !== 1" type="primary" size="small" @click="stateChange(1)">待成交订单总数</el-button>
          </el-badge>
          <el-badge :value="listQuery.state === 4 ? totalDetail.all : ''" class="item" type="primary">
            <el-button :plain="listQuery.state !== 4" type="primary" size="small" @click="stateChange(4)">总部确认驳回</el-button>
          </el-badge>
          <el-badge :value="listQuery.state === 5 ? totalDetail.all : ''" class="item" type="primary">
            <el-button :plain="listQuery.state !== 5" type="primary" size="small" @click="stateChange(5)">待签约</el-button>
          </el-badge>
        </div>
      </div>
    </sticky>
        ...
        <template slot-scope="scope">
          ...
          <el-row :gutter="20">
            ...
            <el-col :span="4" class="btnBox">
              ...
              <el-button v-permission="['/order/orderManager/createNewOrder']" v-if="scope.row.stateName === '确认驳回'" type="primary" plain size="mini" @click="goSigning(scope.row.bidId, scope.row.driverId, scope.row.orderId)">重新提交</el-button>
              <el-button v-permission="['/order/orderManager/createNewOrder']" v-if="scope.row.state === 1" type="primary" size="mini" plain @click="goSigning(scope.row.bidId, scope.row.driverId, scope.row.orderId)">签约成交</el-button>
...
import { GetOrderListNotyet, orderDelete } from '@/api/order/order-api'
...
export default {
  ...
  methods: {
    ...
    getList($state) {
      ...
      GetOrderListNotyet(this.listQuery).then(response => {
