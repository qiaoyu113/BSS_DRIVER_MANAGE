    ...
    <sticky class-name="sub-navbar">
      <div>
        <el-badge :value="totalDetail.all" class="item" type="primary">
          <el-button :plain="stateBtn !== 1" type="primary" size="small" @click="stateChange(1)">标书总数</el-button>
        </el-badge>
      </div>
    </sticky>
        ...
        <template slot-scope="scope">
          ...
          <el-row :gutter="30">
            <el-col :span="7">
              ...
              <p>{{ scope.row.lineName }}</p><p class="el-button--primary is-plain no-bg"><span class="noLight-font">标书状态：</span><span class="val">{{ scope.row.tenderStateName }}</span></p>
            </el-col>
            <el-col :span="7">
              <p><span class="light-font">货主报价：</span>{{ scope.row.lowestQuotations }}元</p>
              <!--<p>{{ scope.row.carTypeName }}/{{ scope.row.cargoType }}</p><p class="el-button&#45;&#45;primary is-plain no-bg"><span class="noLight-font">可上车数：</span><span class="val">{{ scope.row.deployNum }}</span></p>-->
            ...
            <el-col :span="3" class="btnBox">
              <!--<el-button v-permission="['/match/tender/chooseDriver']" v-if="scope.row.tenderStateName === '待中标'" type="primary" plain size="mini" @click="changeDriver(scope.row.tenderId)">选择司机</el-button>-->
...
import { GetAllTenderList, GetChooseDrive, getDriverListForChooseDriver, offLine, JudgeOrderByDriver } from '@/api/tender/tender-api'
...
export default {
  ...
  methods: {
    ...
    getList($state) {
      ...
      GetAllTenderList(this.listQuery).then(response => {
