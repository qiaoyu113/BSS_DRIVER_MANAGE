  ...
  <div class="lineListContainer">
    ...
    <div class="freightDetail">
      <div class="title ellipsis">
        {{ obj.name }}
        <span v-if="obj.lineState === 1" class="prices">{{ obj.lineFee || 0 }}元</span>
        <span v-else class="states">{{ obj.lineStateName }}</span>
      </div>
      ...
      <div class="danceng" style=" border-radius:5px">
        <van-form :show-error="false" @submit="footer_confirm">
          ...
          <p class="footer_but">
            ...
            <van-button v-preventreclick native-type="button" @click="opNoCarBatchBySale">
              未出车
            </van-button>
...
export default {
  ...
  methods: {
    ...
    footer_confirm() {
      ...
      this.subReportMoneyBatchBySale(wayBillAmountIds, gmFee) // 加盟运费上报
    },
    async subReportMoneyBatchBySale(wayBillAmountIds, gmFee) {
      try {
        let parmas = {
          moneys: gmFee, // 运费
