<template>
  <div class="ActiveLineContainer">
    ...
    <StepTwo v-show="step === 2" type="active" :max-date="stepOneForm.waitDirveValidity ? new Date(stepOneForm.waitDirveValidity) : new Date(2125, 12, 31)" :form="stepTwoForm" @stepThree="step=3" @step-one="step=1" />
...
import { getLineDetail, activeStableLine, activeTemporaryLine } from '@/api/line'
  ...
  methods: {
    ...
    handleSubmit() {
      ...
      if (this.isStable) {
        ...
        this.activeTemporaryLine(params)
      }
    },
    // 激活稳定线路
    async activeStableLine(params) {
      try {
        this.$loading(true)
        params.lineCategory = 1
        let { data: res } = await activeStableLine(params)
        if (res.success) {
          this.createSuc()
        } else {
    ...
    // 激活临时线路
    async activeTemporaryLine(params) {
      try {
        this.$loading(true)
        params.lineCategory = 2
        let { data: res } = await activeTemporaryLine(params)
        if (res.success) {
          this.createSuc()
        } else {
          this.$fail(res.errorMsg)
